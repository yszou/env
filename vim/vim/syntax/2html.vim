" --------------------------------------------- changes
" _todo_  let s:title=expand("%:t")
"         exe 'new /tmp/'. s:title . ".html"
" _todo_  exe "normal! a<head>\n<title>" . s:title . "</title>\n\e"
" --------------------------------------------- settings
...
"                    syntax/2html.vim
" ~/svn/vim/vimfiles/syntax/2html.vim
" ---------------------------------------------------------------------------
" for f in *;do gvim -f +"syn on" +"run! syntax/2html.vim" +"wq" +"q" $f;done
" ---------------------------------------------------------------------------
...
" else
"   new %.html
  new /tmp/Untitled.html
else
" -------------------------------------- _todo_
  let s:title = expand("%:p:t")
  exe 'new  /tmp/'. s:title . ".html"
" --------------------------------------
endif
let s:newwin = winnr()
